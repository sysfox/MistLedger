...
src/
  app/                  # Routes (all pages are server components unless noted)
    ...
    loading.tsx         # Route-level structure skeleton mirroring "/" overview layout
                        # (hero + panel/chart skeleton blocks, .skeleton pulse,
                        # sr-only label, no lamp dot or visible text)掌灯�?
