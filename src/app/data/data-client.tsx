...
type DataPayload = {
  days: number;
  accounts: { id: string; name: string; initial_balance: number; is_active: boolean }[];
  categories: { id: string; name: string; kind: string }[];
  stats: FilteredTxStats | null;
  transactions: Tx[];
...
function ResultsSummary({ stats }: { stats: FilteredTxStats | null }) {
  const summary = {
            ...
            <li key={t.id} className="flex items-center justify-between gap-3 px-2 py-2 text-sm">
              <div className="min-w-0">
                ...
                <p className="text-xs text-dim">
                  {t.date} · {fromAcc} · {channelLabel(t.channel ?? undefined)}
...
function TrendSection({ snapshot }: { snapshot: Snapshot | null }) {
...
function AssetCurve({ days, today, snapshot }: { days: number; today: string; snapshot: Snapshot | null }) {
...
export default function DataClient() {
  ...
  apiQs.set("days", String(days));
  if (effective.from) apiQs.set("from", effective.from);
  if (effective.to) apiQs.set("to", effective.to);
  if (effective.type && effective.type !== "all") apiQs.set("type", effective.type);
  if (effective.category) apiQs.set("cat", effective.category);
  if (effective.account) apiQs.set("acc", effective.account);
  if (effective.min != null) apiQs.set("min", String(effective.min));
  if (effective.max != null) apiQs.set("max", String(effective.max));
  if (effective.q) apiQs.set("q", effective.q);
  const apiPath = `/api/data?${apiQs.toString()}`;

  const { data, error, loading } = useApiData<DataPayload>(apiPath);
  ...
  return (
    <main className="mx-auto flex w-full max-w-4xl flex-col gap-6 px-4 py-6">
      ...
      {loading ? (
        ...
        <TrendSection snapshot={data?.snapshot ?? null} />
      )}
      ...
      <section className="panel p-5">
        ...
        {loading ? (
          ...
          <AssetCurve days={days} today={today} snapshot={data?.snapshot ?? null} />
        )}
      </section>
      ...
      <section className="panel p-5">
        ...
        {loading ? (
          ...
          <QueryForm
            ...
            categories={(data?.categories ?? []).map((c) => ({ id: c.id, name: c.name, kind: c.kind }))}
            days={days}
            current={current}
          />
        )}
      ...
      <section id="results" className="panel p-5">
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          ...
          {loading ? (
            ...
            <ResultsSummary stats={data?.stats ?? null} />
          )}
        </div>
